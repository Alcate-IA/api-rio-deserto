...
public class GeralController {
    ...
    @Operation(
            summary = "Obter contadores do sistema",
            description = "Retorna os contadores de registros ativos do sistema Zeus e RDLab"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Contadores obtidos com sucesso",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = Map.class),
                            examples = @ExampleObject(
                                    name = "Contadores",
                                    value = "{\"contadoresZeus\": 150, \"contadoresRdLab\": 120}"
                            )
                    )
            ),
            @ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    name = "Erro",
                                    value = "{\"timestamp\": \"2024-12-13T10:30:00\", \"status\": 500, \"error\": \"Internal Server Error\", \"message\": \"Erro ao consultar contadores\"}"
                            )
                    )
            )
    })
